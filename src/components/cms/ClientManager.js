...
export default function ClientManager() {
  ...
  return (
    <div className="min-w-0">
      <section className="min-w-0 overflow-hidden rounded-2xl border border-deep-navy/10 bg-white p-4 shadow-[0_12px_28px_rgb(23_36_61_/_8%)] sm:p-6" aria-labelledby="client-list-title">
        <div className="flex items-start justify-between gap-4 border-b-2 border-deep-navy/10 pb-5">
          ...
          <button className="rounded-full border border-deep-navy bg-orange px-3 py-2 text-xs font-black uppercase tracking-[0.08em] text-deep-navy shadow-[0_8px_18px_rgb(23_36_61_/_12%)] transition hover:bg-white" type="button" onClick={startNewClient}>
            Tambah
          </button>
        </div>
        ...
        <div className="mt-5 grid gap-2">
          {clients.map((client) => (
            <button
              className={`flex min-w-0 items-center gap-3 rounded-xl border p-3 text-left transition ${form.id === client.id ? "border-deep-navy bg-brand-surface" : "border-transparent bg-slate-50 hover:border-brand-blue"}`}
              key={client.id}
              type="button"
              onClick={() => selectClient(client)}
            >
              <span className="flex h-14 w-16 shrink-0 items-center justify-center rounded-lg bg-white p-1">
      ...
      {isEditorOpen ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-deep-navy/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="client-editor-title">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="rounded-full border border-deep-navy bg-orange px-5 py-3 text-sm font-black uppercase tracking-[0.08em] text-deep-navy shadow-[0_8px_18px_rgb(23_36_61_/_12%)] transition hover:bg-white disabled:opacity-60" disabled={isSaving} type="submit">
              {isSaving ? "Menyimpan..." : "Simpan mitra"}
            </button>
            {form.id ? <button className="rounded-full border border-hot-pink px-5 py-3 text-sm font-black uppercase tracking-[0.08em] text-hot-pink transition hover:bg-hot-pink hover:text-white" type="button" onClick={handleDelete}>Hapus</button> : null}
          </div>
            </form>
          </section>
        </div>
