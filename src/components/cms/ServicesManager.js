        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-deep-navy/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-labelledby="service-editor-title">
          <section className="relative max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-deep-navy/10 bg-white p-4 text-deep-navy shadow-2xl sm:max-h-[92vh] sm:p-7">
            ...
            <form className="mt-6 grid gap-5" onSubmit={handleSubmit}>
              ...
              <label className="flex items-center gap-3 text-sm font-bold"><input checked={form.flashSale} className="h-4 w-4 accent-[var(--brand-deep-navy)]" type="checkbox" onChange={(event) => updateForm("flashSale", event.target.checked)} /> Nyalakan promo Flash Sale</label>
              {form.flashSale ? <div className="rounded-xl border border-hot-pink/30 bg-hot-pink/5 p-4"><p className="text-sm font-black text-deep-navy">Pengaturan Flash Sale</p><p className="mt-1 text-xs leading-5 text-slate-600">Harga awal akan dicoret dan harga jual terbaru akan ditampilkan kepada pelanggan.</p><div className="mt-4 grid gap-5 sm:grid-cols-3"><label className="grid gap-2 text-sm font-bold sm:col-span-1">Harga jual terbaru<input className={inputClassName} required type="text" inputMode="numeric" pattern="[0-9]+" value={form.price} onChange={(event) => updateForm("price", priceInputValue(event.target.value))} placeholder="999000" /></label><label className="grid gap-2 text-sm font-bold">Diskon otomatis (%)<input className={`${inputClassName} bg-slate-100 text-slate-600`} readOnly value={calculateDiscount(form.originalPrice, form.price)} aria-label="Persentase diskon otomatis" /></label><label className="grid gap-2 text-sm font-bold">Berakhir pada<input className={inputClassName} type="datetime-local" value={form.flashSaleEndsAt ? form.flashSaleEndsAt.slice(0, 16) : ""} onChange={(event) => updateForm("flashSaleEndsAt", event.target.value ? `${event.target.value}:00+07:00` : "")} /></label></div><p className="mt-3 text-xs leading-5 text-slate-500">Tulis angka saja. Titik dan simbol rupiah akan diabaikan. Persentase dihitung otomatis dari harga sebelum promo dan harga jual terbaru.</p></div> : null}
