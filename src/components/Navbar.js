...
export default function Navbar({ content }) {
  ...
  const labels = navbarContent.links;
  const tickerItems = navbarContent.tickerItems;

  return (
    <nav
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        isScrolled || isMenuOpen
          ? "border-b border-white/15 bg-deep-navy shadow-lg"
          : "bg-deep-navy"
      }`}
    >
      <div className="container mx-auto flex min-h-[76px] items-center justify-between gap-4 px-4 py-2 sm:min-h-[88px] sm:px-6 sm:py-3 lg:px-8 xl:px-12">
        ...
        <div className="hidden items-center gap-7 text-[16px] font-normal text-white xl:gap-10 lg:flex">
          <ScrollLink
            className="relative rounded-full px-3 py-2 transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/10 hover:text-brand-blue-light"
            href="#"
          >
            {labels[0]}
          </ScrollLink>
        ...
        <div className="hidden shrink-0 items-center gap-4 lg:flex">
          <ScrollLink
            className="rounded-full bg-orange px-5 py-3 text-[13px] font-black uppercase tracking-[0.08em] text-deep-navy transition duration-200 hover:bg-white"
            href="#konsultasi"
          >
            {navbarContent.consultationCta}
          </ScrollLink>
        </div>
      ...
      {isMenuOpen ? (
        <div className="absolute left-0 right-0 top-full border-t border-white/15 bg-deep-navy/95 px-4 py-4 shadow-lg backdrop-blur-md lg:hidden">
          <div className="mx-auto flex max-w-[1240px] flex-col gap-1 text-sm font-bold text-white">
            <ScrollLink className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="#" onClick={closeMenu}>{labels[0]}</ScrollLink>
            <ScrollLink className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="#tentang-kami" onClick={closeMenu}>{labels[1]}</ScrollLink>
            <ScrollLink className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="#layanan" onClick={closeMenu}>{labels[2]}</ScrollLink>
            <ScrollLink className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="#mitra" onClick={closeMenu}>{labels[3]}</ScrollLink>
            <ScrollLink className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="#studi-kasus" onClick={closeMenu}>{labels[4]}</ScrollLink>
            <Link className="rounded-xl px-3 py-3 transition hover:bg-white/10 hover:text-brand-blue-light" href="/tim" onClick={closeMenu}>{labels[5]}</Link>
            <ScrollLink className="mt-2 border-2 border-deep-navy bg-orange px-3 py-3 text-center font-black uppercase tracking-[0.08em] text-deep-navy shadow-[3px_3px_0_var(--brand-hot-pink)] transition hover:bg-white" href="#konsultasi" onClick={closeMenu}>{navbarContent.consultationCta}</ScrollLink>
          </div>
        </div>
      ) : null}
    </nav>
